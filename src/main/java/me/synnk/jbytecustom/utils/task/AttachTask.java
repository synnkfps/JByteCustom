...
import me.synnk.jbytecustom.utils.attach.InjectUtils;
...
public class AttachTask extends SwingWorker<Void, Integer> {
    ...
    public AttachTask(JByteCustom jbm, VirtualMachine vm) {
        this.vm = vm;
        this.jpb = jbm.getPP();
    }
    ...
    protected Void doInBackground() throws Exception {
        ...
        File self = new File(JByteCustom.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath());
        if (self.getAbsolutePath().endsWith(".jar")) {
            JOptionPane.showMessageDialog(null, JByteCustom.res.getResource("injecting_msg"));
            ...
            output.putNextEntry(e);
            output.write(("Manifest-Version: 1.0\nAgent-Class: " + JByteCustom.class.getName()
                    + "\nCan-Redefine-Classes: true\nCan-Retransform-Classes: true\nCan-Set-Native-Method-Prefix: false\n").getBytes());
            output.closeEntry();
            jbytemod.close();
            output.close();
            publish(100);
            JByteCustom.instance.dispose();
            vm.loadAgent(temp.getAbsolutePath(), self.getParent());
            temp.deleteOnExit();
        } else {
            JOptionPane.showMessageDialog(null, JByteCustom.res.getResource("no_jar_found"));
        }
        publish(100);
        return null;
    }
