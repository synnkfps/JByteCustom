package me.synnk.jbytecustom.utils.attach;
...
public class RuntimeJarArchive extends JarArchive {
    ...
    public RuntimeJarArchive(Instrumentation ins) {
        ...
        try {
            loadNames(JByteCustom.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath());
            loadNames(JarUtils.getRT().getAbsolutePath());
            JByteCustom.LOGGER.log("Successfully loaded system class names");
        } catch (Exception e1) {
            e1.printStackTrace();
        }
    }
