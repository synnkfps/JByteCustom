...
public class PluginManager {
    ...
    private File pluginFolder = new File(JByteCustom.workingDir, "plugins");

    public PluginManager(JByteCustom jbm) {
        if (pluginFolder.exists() && pluginFolder.isDirectory()) {
            loadPlugins();
        } else {
            JByteCustom.LOGGER.err("No plugin folder found!");
        }
    }
        ...
        for (File f : pluginFolder.listFiles()) {
            if (f.getName().endsWith(".jar")) {
                try {
                    ...
                    e.printStackTrace();
                    JByteCustom.LOGGER.err("Plugin " + f.getName() + " failed to load!");
                }
            }
        }
        JByteCustom.LOGGER.log(plugins.size() + " plugin(s) loaded!");
    }
