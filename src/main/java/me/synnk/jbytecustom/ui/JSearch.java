...
public class JSearch extends JDialog implements ActionListener {
    ...
    public JSearch(MyCodeList list) {
        ...
        boxes.add(mc = new JCheckBox(JByteCustom.res.getResource("case_sens")));
        boxes.add(ww = new JCheckBox(JByteCustom.res.getResource("whole_word")));
        cp.add(border, BorderLayout.NORTH);
        JPanel bottom = new JPanel(new GridLayout(1, 0, 10, 10));
        JButton findBtn = new JButton(JByteCustom.res.getResource("find_next"));
        findBtn.addActionListener(this);
        bottom.add(findBtn);
        JButton closeBtn = new JButton(JByteCustom.res.getResource("close"));
