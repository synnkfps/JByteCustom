...
public class FernflowerDecompiler extends Decompiler implements IBytecodeProvider, IResultSaver {
    ...
    public FernflowerDecompiler(JByteCustom jbm, DecompilerPanel dp) {
        super(jbm, dp);
    }
    ...
    public String decompile(byte[] b, MethodNode mn) {
        try {
            ...
            for (String key : options.keySet()) {
                map.put(key, JByteCustom.ops.get("ff_" + key).getBoolean() ? "1" : "0");
            }
            Fernflower f = new Fernflower(this, this, map, new PrintStreamLogger(JByteCustom.LOGGER));
