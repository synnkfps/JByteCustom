...
public class KrakatauDecompiler extends Decompiler {
    ...
    public KrakatauDecompiler(JByteCustom jbm, DecompilerPanel dp) {
        super(jbm, dp);
    }
    ...
    public String decompile(byte[] b, MethodNode mn) {
        try {
            ...
            if (decompile == null) {
                decompile = makeTemp();
                JByteCustom.LOGGER.log("Successfully created Krakatau temp folder");
            }
            ...
            while ((s = input.readLine()) != null) {
                JByteCustom.LOGGER.log(s);
            }
            while ((s = error.readLine()) != null) {
                JByteCustom.LOGGER.err(s);
            }
            p.waitFor();
            JByteCustom.LOGGER.log("Finished Krakatau emulation");
            ...
            return sw.toString() + "\n\n" + JByteCustom.res.getResource("set_py_path");
        }
    }

    private String getPythonPath() {
        String pp = JByteCustom.ops.get("python_path").getString();
        if (!pp.isEmpty()) {
            try {
                File path = new File(pp);
                if (!path.exists()) {
                    JByteCustom.LOGGER.err("Python executable does not exist");
                } else {
                    return "\"" + path.getAbsolutePath() + "\"";
                }
            } catch (Exception e) {
                JByteCustom.LOGGER.err("Invalid python path (" + e.toString() + ")");
            }
        }
        return "py";
    }
